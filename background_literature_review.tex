...
%Their system includes five entities, namely: the Certificate Authority, Attribute Authorities, Cloud Server, Data Owners and Data Users. 

%Kan et al.'s framework includes nine algorithms in total:

%\begin{itemize}
%	
%	\item CASetup - The Certificate Authority Setup algorithm, which is run by the CA and takes as input the system security parameter and produces as output: the master key MSK; the global system parameters SP; a secret and verification key pair for the CA; and a global public/secret key pair and certificate for each user who has been assigned a GUID.
%	
%	\item AASetup - The Attribute Authority Setup algorithm, which is run by the individual AAs and takes as input the unique id of the AA and produces as output: a secret/public key pair for the AA; and a set of version keys and public attribute keys for all the attributes that could be issued by the AA.
%	
%	\item SKeyGen - The secret key generation algorithm is run by each AA. It takes as input: a set of attributes that describes the secret key; the authority secret key; the set of public attribute keys; the system parameter; and the certificate of the user with GUID. It outputs a secret key for the user with GUID.
%	
%	\item Encrypt - The encryption algorithm is run by data owners. It takes as input: the system parameter; a set of public keys from the involved authority set; a set of public attribute keys; a message; and an access structure over all the selected attributes from the involved AAs. The algorithm encrypts the message according to the access structure and outputs a ciphertext.
%	
%	\item TKGen -  The decryption token generation algorithm is run by the cloud server. It takes as input the ciphertext which contains an access structure, the user’s global public key and a set of the user’s secret keys. If the set of attributes satisfies the access structure, the algorithm can successfully compute the correct decryption token of the ciphertext.
%	
%	\item Decrypt - The decryption algorithm is run by the users. It takes as inputs the ciphertext, the decryption token and the user’s global secret key. It outputs the message.
%	
%	\item UKeyGen - The update key generation algorithm is run by the AA corresponding to the revoked attribute. It takes as input: the authority secret key; a set of users' secret and the previous version key of the revoked attribute. It outputs both the users' Key Update Key and the Ciphertext Update Key
%	
%	\item SKUpdate - The user’s secret key update algorithm is run by all the non-revoked users. It takes as input the current secret key  and its key update key. It outputs a new secret key.
%	
%	\item CTUpdate - The ciphertext update algorithm is run by the cloud server. It takes as input the current ciphertext and the ciphertext update key . It outputs a new ciphertext. 
%	
%\end{itemize}
