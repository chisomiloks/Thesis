...
%In order to keep up with technological advancements such as cloud computing and take advantage of its numerous advantages, health institutions need to ensure that the security and privacy of their data is assured. This is as a result of the fact that cloud based systems reside in a different trust domain from the users because the users have no physical control of the hardware on which their system is stored. This has severe implications on security and privacy.

%Several cryptographic protocols have been developed and can be used for secure data storage on the cloud. Attribute Based Encryption (ABE) offers data owners better flexibility. ABE allows data owners to provide fine-grained access control over their data stored in its encrypted format in such a way that authorized users are only permitted access to only subsets of data in the overall data base that they have been authorized for and also the system administrators who have physical control of the hardware on which the data is stored have no way of accessing the data. 
